# Offene Punkte

## Diese Woche

- Feminin Aura Webseite live: https://realistisch2020-sys.github.io/FEMCODE-Gehirn/ ✓
- Modul 2: 30 Skripte in 6 Kategorien eingebaut ✓
- **Name entscheiden**: "Feminin Aura" behalten oder umbenennen? (FemSog System, Feminin Sog, DupliFem) → Petra entscheidet
- Link an erste 2-3 Frauen schicken und Feedback sammeln
- Launch Post auf Instagram und Facebook veröffentlichen

## Demnächst

- Elopage Account erstellen (wenn bereit für automatischen Verkauf): elopage.com → kostenlos registrieren → Produkt "Feminin Aura System" anlegen → CHF 37/Monat
- clevver.io aufrufen → virtuelle Geschäftsadresse ab EUR 9.90/Monat → Adresse in AGB einsetzen (outputs/feminin-aura-agb.md)
- AGB: fertig, nur Adresse fehlt noch (wartet auf clevver.io)
- Datum für erstes Praxis Training festlegen

## Ideen / später

- Empfehlungssystem aktivieren (CHF 10 pro geworbene Freundin)
- Preis erhöhen sobald System läuft
- Weitere monatliche Social Media Posts ausbauen
- Video-Willkommensnachricht aufnehmen
- Business-Videos (Modul 7) von Zoom herunterladen → auf YouTube "Nicht gelistet" hochladen → Zoom-Links in Modul 7 durch YouTube-Links ersetzen (Zoom-Links laufen ab!)

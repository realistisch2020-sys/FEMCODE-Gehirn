# -*- coding: utf-8 -*-
"""Journal-Vollwrap fuer 'Du brauchst kein letztes Gespraech', gleicher Stil
wie buch5-journal-cover.py: Vorderseite sauberes Typo-Layout, Ruecken ohne
Text, Rueckseite Blurb. Duesterblau/Gold/Terracotta-Palette passend zum
Hauptbuch-Cover."""
...
OUT_PDF = "/home/user/FEMCODE-Gehirn/outputs/buch-letztes-gespraech/journal-Cover-FullWrap.pdf"
OUT_PNG = "/home/user/FEMCODE-Gehirn/outputs/buch-letztes-gespraech/journal-Cover-FullWrap.png"

INK = HexColor('#1A2137')     # Duesterblau vom Hauptbuch-Cover
GOLD = HexColor('#C9A24A')
TERRA = HexColor('#DFA083')
...
for line in ["Du brauchst kein letztes", "Gespräch, das Journal"]:
    ...
    y -= 9 * mm
y -= 6 * mm
y = wrapped("Ein Buch kann dir zeigen, warum du noch wartest. Aber verändern "
            "tut sich etwas erst dort, wo du selbst schreibst.",
            margin, y, text_w, 'Sans', 11, 5.2 * mm, WHITE)
y -= 5 * mm
y = wrapped("Zu jedem der 12 Kapitel findest du eine kurze Einordnung, "
...
y = H - (BLEED + 34) * mm
c.setFont('Serif-Bold', 18)
c.setFillColor(CREAM)
for line in ["Du brauchst kein", "letztes Gespräch"]:
